# Documentation for the `SimplePackageManager.sh` script.
...
#### [`Thorium Browser SSE3 and AVX2 (currently unavalible)`](https://thorium.rocks) A fork of Chromium that aims for speed and privacy. SSE3 for older CPUs and AVX2 for newer CPUs, use [`CPU-X`](https://aur.archlinux.org/packages/cpu-x) which will be covered later to find out what technologies your CPU supports.
...
#### [`VSCodium`](https://vscodium.com) An open source (better) alternitive Microsoft's closed source [Visual Studio Code](https://code.visualstudio.com/), with telemetry from microsoft removed.
